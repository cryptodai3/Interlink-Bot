...
async function runBot() {
  try {
    ...
    while (true) {
      const claimStatus = await checkIsClaimable(apiClient);

      if (claimStatus.isClaimable) {
        logger.loading(`Claim available. Attempting to claim...`);
        await claimAirdrop(apiClient);

        tokenInfo = await getTokenBalance(apiClient);
        displayUserInfo(userInfo, tokenInfo);
      } else {
        const waitTime = claimStatus.nextFrame - Date.now();
        logger.info(`Next claim not available. Waiting ${formatTimeRemaining(waitTime)}...`);
